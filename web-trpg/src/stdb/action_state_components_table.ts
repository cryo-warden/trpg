...
export class ActionStateComponentsTableHandle {
  ...
  /**
   * Access to the `id` unique index on the table `action_state_components`,
   * which allows point queries on the field of the same name
   * via the [`ActionStateComponentsIdUnique.find`] method.
   *
   * Users are encouraged not to explicitly reference this type,
   * but to directly chain method calls,
   * like `ctx.db.actionStateComponents.id().find(...)`.
   *
   * Get a handle on the `id` unique index on the table `action_state_components`.
   */
  id = {
