...
/**
 * Table handle for the table `ep_components`.
 *
 * Obtain a handle from the [`epComponents`] property on [`RemoteTables`],
 * like `ctx.db.epComponents`.
 *
 * Users are encouraged not to explicitly reference this type,
 * but to directly chain method calls,
 * like `ctx.db.epComponents.on_insert(...)`.
 */
export class EpComponentsTableHandle {
  ...
  /**
   * Access to the `entityId` unique index on the table `ep_components`,
   * which allows point queries on the field of the same name
   * via the [`EpComponentsEntityIdUnique.find`] method.
   *
   * Users are encouraged not to explicitly reference this type,
   * but to directly chain method calls,
   * like `ctx.db.epComponents.entityId().find(...)`.
   *
   * Get a handle on the `entityId` unique index on the table `ep_components`.
   */
  entityId = {
